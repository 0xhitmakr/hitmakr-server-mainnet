import { verifierCreateDSRC, MAX_URI_LENGTH, BASIS_POINTS, MAX_RECIPIENTS } from '../services/contracts/hitmakrdsrcfactory/hitmakrDSRCFactory.js';
...
export async function createDSRC(req, res) {
   try {
       ...
       const {tokenURI, uploadHash, collectorsPrice, licensingPrice, recipients, percentages, selectedChain} = dsrcData;

       if (!tokenURI || !uploadHash || !recipients || !percentages || !Array.isArray(recipients) || !Array.isArray(percentages) || !selectedChain) {
       ...
       if (BigInt(formattedCollectorsPrice) < 0 || BigInt(formattedLicensingPrice) < 0) {
           return res.status(400).json({
               success: false,
       ...
       if (BigInt(formattedCollectorsPrice) === 0n && BigInt(formattedLicensingPrice) === 0n) {
           return res.status(400).json({
               success: false,
